...
export default function IndiaLedger(){const[active,setActive]=useState(0);const point=points[active];const Icon=point.icon;return <section className={styles.section} aria-labelledby="india-preview-title">
 <div className={styles.copy} data-reveal="left"><p>Paper across India</p><h2 id="india-preview-title">One country. <em>Many fibre stories.</em></h2><span>The map is not just geography. It is a field transcript of growers, mills, users, collectors and the decisions that connect them.</span><div className={styles.numbers}><div><strong>75%</strong><small>recycled fibre share</small></div><div><strong>16 kg</strong><small>paper use per person</small></div><div><strong>860+</strong><small>mills nationwide</small></div></div><div><Link href="/india-map">Open the Fibre Map <ArrowRight/></Link><Link href="/india-snapshot">See facts &amp; numbers <ArrowRight/></Link></div><aside><small>Map note 04</small><p>Tap a pin or heat zone to read a regional material field note.</p></aside></div>
 <div className={styles.atlas} data-reveal="right"><header><span><MapPinned/> India fibre atlas</span><small><i/> Higher paper activity</small></header><div className={styles.map}><Image src="/images/maps/india-state-outline.png" alt="State outline map of India with highlighted paper activity regions" fill sizes="(max-width: 850px) 90vw, 620px"/><div className={styles.heatLayer} aria-hidden="true"><i className={styles.heatNorth}/><i className={styles.heatWest}/><i className={styles.heatEast}/><i className={styles.heatSouth}/></div>{points.map((item,index)=><button className={`${styles.pin} ${styles[`pin${index+1}`]} ${active===index?styles.active:""}`} onClick={()=>setActive(index)} aria-label={`Show ${item.label}`} aria-pressed={active===index} key={item.label}><span>{index+1}</span></button>)}</div><article key={point.label}><Icon/><div><small>{point.label} · {point.metric}</small><strong>{point.title}</strong><p>{point.fact}</p></div></article></div>
