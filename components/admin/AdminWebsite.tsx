...
export default function AdminWebsite() {
  ...
  async function save() {
    ...
    const responses = await Promise.all(records.map(record => fetch("/api/settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(record) })));
    const succeeded = responses.every(response => response.ok);
  ...
  return <div>
    <header className="admin-workspace-heading"><div><p>Whole website control</p><h1>Website</h1><span className={`admin-change-state ${dirty ? "has-changes" : ""}`}><i />{dirty ? "Unsaved changes" : "Everything saved"}</span></div><div><button className="admin-action" disabled={!dirty} onClick={undo}><RotateCcw /> Undo changes</button><button className="admin-action" onClick={() => void load()}><RefreshCw /> Reload</button><Link className="admin-action" href="/" target="_blank"><ExternalLink /> View site</Link><button disabled={!dirty} className="admin-action primary" onClick={() => void save()}><Save /> Publish changes</button></div></header>
    {notice && <div className="admin-notice"><Check /> {notice}</div>}
    <section className="admin-easy-guide"><Sparkles /><div><strong>Simple editing rule</strong><p>Change the words on the left, reorder or hide homepage sections on the right, preview the public site, then publish once.</p></div><span><Info /> Changes stay private until published</span></section>
    <div className="admin-website-grid">
      <section className="admin-panel admin-global-controls"><header><div><p>GLOBAL CONTROLS</p><h2>Identity, navigation and footer</h2></div></header><div className="admin-form">
        <Field label="Brand name" help="The main name shown in navigation." value={values["public.brand.name"]} onChange={value => set("public.brand.name", value)} />
        <Field label="Country label" help="The smaller location label beside the brand." value={values["public.brand.country"]} onChange={value => set("public.brand.country", value)} />
        <Field label="Join button" value={values["public.navigation.join"]} onChange={value => set("public.navigation.join", value)} />
        <Field label="Contact button" value={values["public.navigation.contact"]} onChange={value => set("public.navigation.contact", value)} />
        <Field full label="Footer reveal words, separated by |" help="Each word becomes one animated reveal." value={values["public.footer.statement"]} onChange={value => set("public.footer.statement", value)} />
        <Field full label="Footer opening note" value={values["public.footer.intro"]} onChange={value => set("public.footer.intro", value)} />
        <Field full label="Footer closing message" value={values["public.footer.body"]} onChange={value => set("public.footer.body", value)} textarea />
        <Field label="Footer action" value={values["public.footer.action"]} onChange={value => set("public.footer.action", value)} />
        <Field label="Official contact email" value={values["public.contact.email"]} onChange={value => set("public.contact.email", value)} />
      </div></section>
      <section className="admin-panel admin-home-order"><header><div><p>HOMEPAGE</p><h2>Order and visibility</h2></div><Link href="/" target="_blank">Preview <ExternalLink /></Link></header><div>{sections.map((section, index) => <article key={section.id}><span>{String(index + 1).padStart(2, "0")}</span><strong>{section.label}</strong><button onClick={() => setSections(current => current.map(item => item.id === section.id ? { ...item, enabled: !item.enabled } : item))}>{section.enabled ? <Eye /> : <EyeOff />}</button><button disabled={index === 0} onClick={() => move(index, -1)}><ArrowUp /></button><button disabled={index === sections.length - 1} onClick={() => move(index, 1)}><ArrowDown /></button></article>)}</div></section>
    </div>
...
function Field({ label, help, value = "", onChange, full = false, textarea = false }: { label: string; help?: string; value?: string; onChange: (value: string) => void; full?: boolean; textarea?: boolean }) {
  return <label className={full ? "full" : ""}><span>{label}</span>{help && <small className="admin-field-help">{help}</small>}{textarea ? <textarea rows={4} value={value} onChange={event => onChange(event.target.value)} /> : <input value={value} onChange={event => onChange(event.target.value)} />}</label>;
}
