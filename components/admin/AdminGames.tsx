...
export default function AdminGames() {
  ...
  return <div>
    ...
    <div className="admin-data-list">{items.map((game, index) => <article className="admin-data-row" key={game.gameId ?? game.id}><div><strong>{game.title}</strong><small>{game.subtitle || game.description}</small></div><span>{stats.find(item => item.gameId === (game.gameId ?? game.id))?.plays ?? 0} plays</span><em>{game.enabled ? "Live" : "Hidden"}</em><div className="admin-row-actions">{canEdit&&<><button onClick={() => void move(index,-1)} aria-label="Move up"><ChevronUp /></button><button onClick={() => void move(index,1)} aria-label="Move down"><ChevronDown /></button></>}<button onClick={() => setEditing({ ...game, gameId: game.gameId ?? game.id ?? "" })} aria-label="Edit"><Edit3 /></button></div></article>)}</div>
    {editing && <div className="admin-drawer-backdrop"><section className="admin-drawer"><header><div><p>{canEdit?"GAME CONFIGURATION":"GAME DETAILS"}</p><h2>{editing.title}</h2></div><button onClick={() => setEditing(null)}><X /></button></header><GameForm game={editing} onSave={canEdit?save:undefined} /></section></div>}
  </div>;
}
...
function GameForm({ game, onSave }: { game: Game; onSave?: (game: Game) => void }) {
  ...
  const [jsonText, setJsonText] = useState(JSON.stringify(game.content ?? {}, null, 2));
  const set = (key: keyof Game, value: unknown) => setForm(current => ({ ...current, [key]: value }));
  return <form className="admin-form" onSubmit={event => { event.preventDefault(); if (onSave && !jsonError) onSave(form); }}>
    ...
    <label className="full"><span>Game content JSON</span><textarea className="code" rows={12} value={jsonText} onChange={event => { setJsonText(event.target.value); try { set("content", JSON.parse(event.target.value)); setJsonError(""); } catch { setJsonError("Enter valid JSON before saving."); } }} />{jsonError&&<small className="admin-field-error">{jsonError}</small>}</label>
    ...
    {onSave&&<footer><button type="button" onClick={() => { setForm(game); setJsonText(JSON.stringify(game.content ?? {}, null, 2)); setJsonError(""); }}>Reset</button><button className="primary" disabled={Boolean(jsonError)}><Save /> Save configuration</button></footer>}
  </form>;
}
