...
type ArticleItem = { _id?: string; title: string; slug: string; category: string; excerpt?: string; summary?: string; body?: string; status?: string; featured?: boolean; format?: string; order?: number; time?: string; coverImage?: string; readingMinutes?: number; revisionNote?: string };
...
export default function ArticleManager() {
  ...
  async function save(data: Record<string, unknown>) {
    ...
    const payload = { ...data, ...(current?._id ? { id: current._id } : {}), format: current?.format ?? "Core lesson", readingMinutes: Number(data.readingMinutes ?? current?.readingMinutes ?? String(current?.time ?? "7").match(/\d+/)?.[0] ?? 7), sources: [], tags: [], order: current?.order ?? items.length };
  ...
  return <div>
    <header className="admin-workspace-heading"><div><p>Editorial publishing</p><h1>Articles</h1></div><div><button className="admin-action" onClick={() => void load()}><RefreshCw /> Refresh</button><button className="admin-action primary" onClick={() => setEditing("new")}><FilePlus2 /> New article</button></div></header>
    {source !== "cms" && <div className="admin-setup-banner"><BarChart3 /><div><strong>Catalog preview mode</strong><p>Connect MongoDB to edit and reorder the source articles from this workspace.</p></div></div>}
    {notice && <div className="admin-notice">{notice}</div>}
    {editing ? <section className="admin-editor-panel"><header><div><p>EDITOR</p><h2>{editing === "new" ? "Create article" : editing.title}</h2></div><button onClick={() => setEditing(null)}>Close</button></header><ArticleEditor initialData={editing === "new" ? undefined : { title: editing.title, slug: editing.slug, category: editing.category, excerpt: editing.excerpt ?? editing.summary ?? "", body: editing.body ?? "", status: editing.status ?? "draft", featured: Boolean(editing.featured), coverImage: editing.coverImage, readingMinutes: editing.readingMinutes, revisionNote: editing.revisionNote }} onSave={save} /></section> : <>
