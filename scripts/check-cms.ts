...
const required = ["MONGODB_URI", "NEXTAUTH_SECRET", "NEXTAUTH_URL", "NEXT_PUBLIC_SITE_URL", "CLOUDINARY_CLOUD_NAME", "CLOUDINARY_API_KEY", "CLOUDINARY_API_SECRET", "GMAIL_CLIENT_ID", "GMAIL_CLIENT_SECRET", "GMAIL_REFRESH_TOKEN", "GMAIL_SENDER_EMAIL", "CRON_SECRET"];
...
async function check() {
  ...
  if (process.env.CRON_SECRET!.length < 32) throw new Error("CRON_SECRET must be at least 32 characters");
  ...
  console.log(JSON.stringify({ ready: true, database: "connected", collections: collections.length, publicUrl: publicUrl.origin, integrations: { cloudinary: "configured", gmail: "configured", scheduler: "secured" } }, null, 2));
}
